...
$sql = "
    SELECT pt.*,
           COALESCE(s.name, s.email, 'Unknown')  AS student_name,
           s.matric_no,
...
$statsQ = db()->prepare("
    SELECT
        COUNT(*)                                                           AS total,
        SUM(pt.payment_status IN ('Paid','paid'))                          AS paid_count,
        SUM(pt.payment_status = 'Pending')                                AS pending_count,
        SUM(CASE WHEN pt.payment_status IN ('Paid','paid') THEN pt.amount ELSE 0 END) AS revenue,
...
$evList = db()->prepare("
    ...
    FROM events e
    LEFT JOIN payment_transactions pt2 ON pt2.event_id = e.id AND pt2.club_id = ?
    WHERE e.club_id = ? OR e.created_by = ? OR pt2.id IS NOT NULL
    ORDER BY e.start_date DESC
");
$evList->execute([$clubId, $clubId, $adminId]);
