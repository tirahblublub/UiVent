<?php
// ============================================================
//  users/events.php — Browse Events (Student Portal)
//  Connects directly to events created via admin/create_event.php
// ============================================================
...
try {
    // ── Fetch all upcoming events + check if student already registered ──
    $stmt = db()->prepare("
        SELECT
            e.id,
            e.title,
        ...
        LEFT JOIN payment_transactions pt
               ON pt.student_id = ?
              AND pt.event_id = e.id
              AND pt.payment_status IN ('Paid','paid','successful','Successful')
        WHERE e.status NOT IN ('cancelled', 'archived')
        ORDER BY e.start_date ASC
    ");
    $stmt->execute([$sid, $sid]);
    $events = $stmt->fetchAll();

    // Tag _registered into each event so JS knows
    foreach ($events as &$ev) {
        $ev['_registered']    = (bool)$ev['already_registered'];
        $ev['_regId']         = $ev['reg_id'] ?? null;
        $ev['_paymentStatus'] = $ev['payment_status'] ?? '';
        $ev['_txnId']         = $ev['txn_id'] ?? null;
    }
    unset($ev);
...
// ── Fallback demo data (only shown when DB is unavailable) ──
if ($dbError || empty($events)) {
    $events = [
        ['id'=>1,'title'=>'UiTM Career Fair 2026','category'=>'Academic','venue'=>'DATC Hall','event_date'=>'14 Jun 2026','event_time'=>'9:00 AM','capacity'=>500,'registered'=>380,'pct'=>76,'description'=>'Connect with top employers, explore career opportunities, and attend industry talks. Dress code: Smart casual.','image_url'=>'https://images.unsplash.com/photo-1560523159-4a9692d222ef?w=600&q=80','status'=>'open','organizer'=>'MPP UiTM Machang','organizer_avatar'=>'','organizer_email'=>'mpp@uitm.edu.my','organizer_phone'=>'','organizer_office'=>'','registration_fee'=>0,'_registered'=>false,'_regId'=>null],
        ['id'=>2,'title'=>'Fatihah Night Arts Festival','category'=>'Cultural','venue'=>'Laman Kreatif','event_date'=>'20 Jun 2026','event_time'=>'7:00 PM','capacity'=>200,'registered'=>0,'pct'=>0,'description'=>'An open-air arts and performance night featuring student theatre groups, live music, and visual art installations.','image_url'=>'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=600&q=80','status'=>'upcoming','organizer'=>'JPK Kolej Tun Hussein Onn','organizer_avatar'=>'','organizer_email'=>'jpk.tho@uitm.edu.my','organizer_phone'=>'','organizer_office'=>'','registration_fee'=>0,'_registered'=>false,'_regId'=>null],
        ['id'=>3,'title'=>'Hari Sukan UiTM 2026','category'=>'Sports','venue'=>'Stadium UiTM','event_date'=>'28 Jun 2026','event_time'=>'8:00 AM','capacity'=>800,'registered'=>540,'pct'=>67,'description'=>'Annual sports day with inter-faculty competitions. Participation earns co-curricular points.','image_url'=>'https://images.unsplash.com/photo-1461896836934-ffe607ba8211?w=600&q=80','status'=>'open','organizer'=>'MPP UiTM Machang','organizer_avatar'=>'','organizer_email'=>'mpp@uitm.edu.my','organizer_phone'=>'','organizer_office'=>'','registration_fee'=>0,'_registered'=>false,'_regId'=>null],
    ];
}
...
.event-card:hover { transform: translateY(-5px); box-shadow: 0 20px 40px rgba(80,0,120,0.15); }
.event-card .card-img { position:relative; height:160px; overflow:hidden; }
.event-card .card-img img { width:100%; height:100%; object-fit:cover; transition:transform .45s ease; }
.event-card:hover .card-img img { transform:scale(1.06); }
.card-img-overlay { position:absolute; inset:0; background:linear-gradient(to bottom, rgba(0,0,0,0) 40%, rgba(15,5,30,0.72) 100%); }
.card-cat-pill { position:absolute; top:10px; left:10px; backdrop-filter:blur(6px); }
.card-status-pill { position:absolute; top:10px; right:10px; backdrop-filter:blur(6px); }
.card-date-label { position:absolute; bottom:10px; left:12px; color:#fff; font-size:.7rem; font-weight:600; text-shadow:0 1px 4px rgba(0,0,0,.6); }

.progress-bar-fill { transition: width 1.1s cubic-bezier(.4,0,.2,1); }

#eventDrawer > div { transform:translateX(100%); transition:transform .32s cubic-bezier(.4,0,.2,1); }
#eventDrawer:not(.hidden) > div { transform:translateX(0); }
.drawer-hero { position:relative; height:200px; overflow:hidden; }
.drawer-hero img { width:100%; height:100%; object-fit:cover; }
.drawer-hero-overlay { position:absolute; inset:0; background:linear-gradient(to top, rgba(10,2,30,.85) 0%, rgba(10,2,30,.25) 100%); }
.drawer-hero-content { position:absolute; bottom:0; left:0; right:0; padding:16px 20px; }
...
.btn-register.registered { background:linear-gradient(135deg,#059669,#10b981); cursor:default; transform:none; }

.db-banner {
  background:linear-gradient(90deg,#fef3c7,#fde68a);
  border:1px solid #f59e0b; border-radius:10px;
  padding:10px 16px; font-size:.75rem; color:#92400e;
  ...
  <div class="flex-1">
    <main class="view-section active p-6 md:p-8 max-w-7xl w-full mx-auto space-y-6">
      ...
      <div class="db-banner">
        ...
        <span>Tidak dapat sambung ke pangkalan data — menunjukkan data demo. Semak kredensial DB anda.</span>
      </div>
      <?php endif; ?>
      ...
      <div class="events-hero px-8 py-7 flex items-center justify-between">
        ...
        <div class="hidden md:flex items-center gap-6 relative z-10">
          <div class="text-center">
            <p class="text-3xl font-extrabold text-white"><?= count($events) ?></p>
      ...
      <div class="bg-white rounded-2xl border border-gray-200 p-4 flex flex-wrap gap-3 items-center shadow-sm">
        ...
        <select id="filterCategory" class="bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-purple-400 transition-colors">
          ...
          <option>Academic</option><option>Cultural</option><option>Sports</option><option>Other</option>
        </select>
...
<div id="eventDrawer" class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 hidden flex justify-end">
  <div class="w-full max-w-lg bg-white h-full shadow-2xl overflow-y-auto flex flex-col">
    ...
    <div class="flex-1 p-6 space-y-5">
      <div class="flex flex-wrap gap-3">
        <div class="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 rounded-xl px-3 py-2">
          <i class="fas fa-map-marker-alt text-amber-500 text-xs"></i><span id="dVenue">Venue</span>
        </div>
        <div class="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 rounded-xl px-3 py-2">
          <i class="far fa-calendar text-purple-400 text-xs"></i><span id="dDate">Date</span>
        </div>
        <div class="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 rounded-xl px-3 py-2">
          <i class="far fa-clock text-purple-400 text-xs"></i><span id="dTime">Time</span>
        </div>
      </div>
      ...
      <div class="bg-purple-50 border border-purple-100 rounded-2xl p-4 space-y-3">
        ...
        <div class="space-y-2">
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="text-xs text-purple-500 font-semibold uppercase tracking-wide">Club / Unit</p>
    ...
    <div class="p-6 border-t border-gray-100 space-y-2.5 sticky bottom-0 bg-white">
      ...
      <!-- Pay Now button — shown only for paid events with pending payment -->
      <form id="dPayForm" method="POST" action="create_bill.php" class="hidden">
        <input type="hidden" name="type"   value="event">
        <input type="hidden" name="ref_id" id="dPayRefId">
        <input type="hidden" name="desc"   id="dPayDesc">
        <input type="hidden" name="amount" id="dPayAmount">
        <button type="submit"
          class="w-full flex items-center justify-center gap-2 font-semibold text-xs py-2.5 rounded-xl transition-colors"
          style="background:#f59e0b;color:#1c1917;">
          <i class="fas fa-credit-card text-xs"></i> Pay Registration Fee
        </button>
      </form>
    ...
    <div class="px-6 py-6 text-center space-y-4">
      ...
      <div class="space-y-2 pt-1">
        ...
        <button onclick="closeQrModal()"
          class="block w-full bg-gray-100 hover:bg-gray-200 text-gray-600 font-medium py-2.5 rounded-xl text-xs transition-colors">Close</button>
      </div>
...
const CAT_CLASS = { academic:'cat-academic', cultural:'cat-cultural', sports:'cat-sports', business:'cat-business', religious:'cat-religious' };
const STATUS_CLASS = { 'open':'status-open', 'upcoming':'status-upcoming', 'filling fast':'status-filling', 'closed':'status-closed' };
const BAR_COLOR = { academic:'#3b82f6', cultural:'#f59e0b', sports:'#ef4444', business:'#10b981', religious:'#6366f1' };
...
const PLACEHOLDERS = {
  ...
  'Other'   :'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=600&q=60',
};

function organizerAvatar(ev, size='w-6 h-6 text-[10px]') {
  if (ev.organizer_avatar) return `<img src="${ev.organizer_avatar}" class="${size} rounded-full object-cover ring-1 ring-white" alt="">`;
  ...
  return `<span class="${size} rounded-full flex items-center justify-center font-bold text-white ring-1 ring-white shrink-0" style="background:${colors[idx]}">${initials(ev.organizer||'?')}</span>`;
}

let currentLayout = 'grid';

function buildCard(ev) {
  const img   = ev.image_url || PLACEHOLDERS[ev.category] || PLACEHOLDERS['Other'];
  const pct   = Math.min(parseInt(ev.pct)||0, 100);
  const isFull = pct >= 100;
  const reg   = !!ev._registered;
  const fee   = parseFloat(ev.registration_fee) || 0;
  const isPaid = ev._txnId != null || ['paid','successful'].includes((ev._paymentStatus||'').toLowerCase());
  const needsPay = reg && fee > 0 && !isPaid;

  const btnLabel = reg
    ? '<i class="fas fa-check text-xs"></i> Registered'
    : (isFull ? 'Full' : (fee > 0 ? `Register — RM ${fee.toFixed(2)}` : 'Register Now'));
  const btnClass = reg ? 'btn-register registered' : 'btn-register';

  const payBadge = needsPay
    ? `<form method="POST" action="create_bill.php" onclick="event.stopPropagation()">
        <input type="hidden" name="type"   value="event">
        <input type="hidden" name="ref_id" value="${ev._txnId || ev._regId}">
        <input type="hidden" name="desc"   value="Registration: ${safe(ev.title)}">
        <input type="hidden" name="amount" value="${fee.toFixed(2)}">
        <button type="submit" class="w-full mt-1 flex items-center justify-center gap-1.5 text-xs font-bold py-2 rounded-xl transition-colors" style="background:#f59e0b;color:#1c1917;">
          <i class="fas fa-credit-card text-xs"></i> Pay RM ${fee.toFixed(2)}
        </button>
      </form>`
    : '';
  ...
  if (currentLayout === 'list') {
    return `
    <div class="event-card bg-white border border-gray-200 shadow-sm flex overflow-hidden cursor-pointer" onclick="openEventDrawer(${ev.id})">
      <div class="w-36 shrink-0 relative">
        <img src="${img}" alt="${safe(ev.title)}" class="w-full h-full object-cover" onerror="this.src='${PLACEHOLDERS['Other']}'">
      ...
      <div class="flex-1 p-4 flex flex-col gap-2">
        ...
        <div>
          ...
          <p class="text-xs text-gray-400 mt-0.5"><i class="fas fa-map-marker-alt mr-1 text-amber-400"></i>${ev.venue} &nbsp;·&nbsp; <i class="far fa-calendar mr-1"></i>${ev.event_date}</p>
          <div class="flex items-center gap-1.5 mt-1.5">${organizerAvatar(ev)}<span class="text-xs text-gray-500 font-medium">${safe(ev.organizer)}</span></div>
        </div>
        <div class="mt-auto">
          <div class="flex justify-between text-xs text-gray-400 mb-1"><span>Registrations</span><span class="font-semibold text-gray-600">${ev.registered} / ${ev.capacity}</span></div>
          <div class="w-full bg-gray-100 rounded-full h-1.5"><div class="h-1.5 rounded-full" style="width:${pct}%;background:${barColor(ev.category)}"></div></div>
        </div>
      </div>
      <div class="flex flex-col items-center pr-4 gap-1">
        <button onclick="event.stopPropagation(); registerEvent(this, ${ev.id})" class="${btnClass} !w-auto px-4 text-xs" ${reg||isFull?'disabled':''}>
          ${btnLabel}
        </button>
        ${needsPay ? `<form method="POST" action="create_bill.php" onclick="event.stopPropagation()">
          <input type="hidden" name="type"   value="event">
          <input type="hidden" name="ref_id" value="${ev._txnId || ev._regId}">
          <input type="hidden" name="desc"   value="Registration: ${safe(ev.title)}">
          <input type="hidden" name="amount" value="${fee.toFixed(2)}">
          <button type="submit" class="text-xs font-bold px-3 py-1.5 rounded-xl" style="background:#f59e0b;color:#1c1917;white-space:nowrap;">
            <i class="fas fa-credit-card text-xs"></i> Pay RM ${fee.toFixed(2)}
          </button>
        </form>` : ''}
      </div>
  ...
  return `
  <div class="event-card bg-white border border-gray-200 shadow-sm flex flex-col cursor-pointer" onclick="openEventDrawer(${ev.id})">
    <div class="card-img">
      <img src="${img}" alt="${safe(ev.title)}" onerror="this.src='${PLACEHOLDERS['Other']}'">
      <div class="card-img-overlay"></div>
    ...
    <div class="p-5 flex flex-col gap-3 flex-1">
      <div>
        <h4 class="font-bold text-gray-900 text-sm leading-snug">${ev.title}</h4>
        <p class="text-xs text-gray-400 mt-1"><i class="fas fa-map-marker-alt mr-1 text-amber-400"></i>${ev.venue}</p>
      </div>
      ...
      <div>
        <div class="flex justify-between text-xs text-gray-400 mb-1.5"><span>Registrations</span><span class="font-semibold text-gray-600">${ev.registered} / ${ev.capacity}</span></div>
        <div class="w-full bg-gray-100 rounded-full h-1.5"><div class="h-1.5 rounded-full" style="width:${pct}%;background:${barColor(ev.category)}"></div></div>
      </div>
      <button onclick="event.stopPropagation(); registerEvent(this, ${ev.id})"
        class="${btnClass} mt-auto" ${reg||isFull?'disabled':''}>
        ${btnLabel}
      </button>
      ${payBadge}
    </div>
...
function setLayoutTab(mode) {
  ...
  const active = 'tab-btn active px-3 py-2.5 text-xs font-semibold';
  const inactive = 'tab-btn px-3 py-2.5 text-xs font-semibold bg-white text-gray-600 hover:bg-gray-50';
  if (mode==='grid') {
    btnG.className=active+' rounded-l-xl'; btnL.className=inactive+' rounded-r-xl';
    grid.className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5';
  } else {
    btnG.className=inactive+' rounded-l-xl'; btnL.className=active+' rounded-r-xl';
...
function openEventDrawer(id) {
  ...
  const dPayForm = document.getElementById('dPayForm');
  const fee      = parseFloat(ev.registration_fee) || 0;
  const isPaid   = ev._paymentStatus === 'Paid' || ev._paymentStatus === 'paid';
  const isPending = ev._registered && fee > 0 && !isPaid;

  // Hide pay form by default
  dPayForm.classList.add('hidden');
  ...
  if (ev._registered) {
    ...
    dBtn.disabled = true; dBtn.onclick = null;

    // Show Pay Now if fee exists and not yet paid
    if (isPending) {
      document.getElementById('dPayRefId').value  = ev._txnId || ev._regId;
      document.getElementById('dPayDesc').value   = 'Registration: ' + ev.title;
      document.getElementById('dPayAmount').value = fee.toFixed(2);
      dPayForm.classList.remove('hidden');
    }

    // Show QR modal for already-registered events
    if (ev._regId) {
      const ref       = 'UV-' + String(ev._regId).padStart(6, '0');
      const qrData    = 'UiVent|' + ref + '|' + ev.title;
      const container = document.getElementById('qrCanvas');
      container.innerHTML = '';
      new QRCode(container, { text: qrData, width: 200, height: 200, colorDark: '#3b0764', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.H });
      document.getElementById('qrEventTitle').innerText = ev.title;
      document.getElementById('qrRegId').innerText      = ref;
      document.getElementById('qrDirectLink').href      = 'mybookings.php';
      document.getElementById('qrModal').classList.remove('hidden');
    }
  } else {
    dBtn.className = 'btn-register flex items-center justify-center gap-2';
    dBtn.innerHTML = fee > 0
      ? '<i class="fas fa-ticket-alt text-xs"></i><span>Register — RM ' + fee.toFixed(2) + '</span>'
      : '<i class="fas fa-ticket-alt text-xs"></i><span>Register for This Event</span>';
    dBtn.disabled = false; dBtn.onclick = drawerRegister;
  }

  const img = ev.image_url || PLACEHOLDERS[ev.category] || PLACEHOLDERS['Other'];
  document.getElementById('dImage').src            = img;
  ...
  document.getElementById('dOrgClub').innerHTML = ev.organizer || NA;

  const dOrgEmail = document.getElementById('dOrgEmail');
  if (ev.organizer_email) { dOrgEmail.href=`mailto:${ev.organizer_email}`; dOrgEmail.innerText=ev.organizer_email; dOrgEmail.style.pointerEvents=''; }
  else { dOrgEmail.innerHTML=NA; dOrgEmail.removeAttribute('href'); dOrgEmail.style.pointerEvents='none'; }

  const dOrgPhone = document.getElementById('dOrgPhone');
  if (ev.organizer_phone) { dOrgPhone.href=`tel:${ev.organizer_phone.replace(/\s/g,'')}`; dOrgPhone.innerText=ev.organizer_phone; dOrgPhone.style.pointerEvents=''; }
  else { dOrgPhone.innerHTML=NA; dOrgPhone.removeAttribute('href'); dOrgPhone.style.pointerEvents='none'; }

  document.getElementById('dOrgOffice').innerHTML = ev.organizer_office || NA;

  const dOrgAvatar = document.getElementById('dOrgAvatar');
  const colors = ['#7c3aed','#059669','#d97706','#dc2626','#2563eb','#db2777'];
  const idx = (ev.organizer||'').split('').reduce((a,c)=>a+c.charCodeAt(0),0)%colors.length;
  if (ev.organizer_avatar) {
    dOrgAvatar.innerHTML=''; dOrgAvatar.style.background='transparent';
    const img2=document.createElement('img'); img2.src=ev.organizer_avatar; img2.className='w-10 h-10 rounded-full object-cover';
    dOrgAvatar.appendChild(img2);
  } else {
    dOrgAvatar.innerHTML=initials(ev.organizer||'?'); dOrgAvatar.style.background=colors[idx];
  }
...
function closeEventDrawer() { document.getElementById('eventDrawer').classList.add('hidden'); }
document.getElementById('eventDrawer').addEventListener('click', function(e){ if(e.target===this) closeEventDrawer(); });

function drawerRegister() { if(currentDrawerEventId) doRegister(currentDrawerEventId, null); }
  ...
  fetch('register_event.php', { method:'POST', body:fd })
    ...
    .then(data=>{
      if (data.success) {
        ...
        if (ev) {
          ev._registered    = true;
          ev._regId         = data.registration_id;
          ev._txnId         = data.txn_id || null;
          ev._paymentStatus = data.fee > 0 ? '' : 'Paid';
          ev.registration_fee = data.fee || ev.registration_fee;
        }
        if (cardBtn)  { cardBtn.className='btn-register registered flex items-center justify-center gap-2'; cardBtn.innerHTML='<i class="fas fa-check text-xs"></i> Registered'; cardBtn.onclick=null; cardBtn.disabled=false; }
        if (drawerBtn){ drawerBtn.className='btn-register registered flex items-center justify-center gap-2'; drawerBtn.innerHTML='<i class="fas fa-check text-xs"></i> Registered'; drawerBtn.disabled=false; drawerBtn.onclick=null; }

        // Re-render cards so Pay button appears immediately without page refresh
        renderEvents(getFiltered());

        // Show Pay Now in drawer if fee exists
        if (ev && data.fee > 0 && data.txn_id) {
          const dPayForm = document.getElementById('dPayForm');
          document.getElementById('dPayRefId').value  = data.txn_id;
          document.getElementById('dPayDesc').value   = 'Registration: ' + (data.event_title || currentDrawerTitle);
          document.getElementById('dPayAmount').value = parseFloat(data.fee).toFixed(2);
          dPayForm.classList.remove('hidden');
        }

        if (!data.already && data.registration_id) {
          closeEventDrawer();
          openQrModal(data.event_title || currentDrawerTitle, data.qr_url, data.registration_id, data.ref);
...
function registerEvent(btn, eventId) { doRegister(eventId, btn); }

function openQrModal(title, qrUrl, regId, ref) {
  document.getElementById('qrEventTitle').innerText = title;
  document.getElementById('qrRegId').innerText      = ref || ('UV-' + String(regId).padStart(6,'0'));
  document.getElementById('qrDirectLink').href      = 'mybookings.php';
  const container = document.getElementById('qrCanvas');
  container.innerHTML='';
  new QRCode(container, { text:qrUrl, width:200, height:200, colorDark:'#3b0764', colorLight:'#ffffff', correctLevel:QRCode.CorrectLevel.H });
  document.getElementById('qrModal').classList.remove('hidden');
}
...
function showToast(msg, isError=false) {
  ...
  document.getElementById('toastIcon').className=isError?'fas fa-exclamation-circle text-red-400':'fas fa-check-circle text-emerald-400';
  el.classList.remove('hidden');
  clearTimeout(toastTimer);
  toastTimer=setTimeout(()=>el.classList.add('hidden'),3800);
}

document.addEventListener('DOMContentLoaded', ()=>{ renderEvents(ALL_EVENTS); });
</script>
</body>
</html>